        ...
        try {
            ...
            $query = M_ListbanData::query()
                ...
                ->with([
                    ...
                    'surveyor:id,fullname,position,keterangan,username',
                    'deploy:ID,NO_SURAT,LOAN_NUMBER,STATUS,CREATED_AT',
                ])
            ...
            if ($user->position !== 'HO') {
                $query->where('BRANCH_ID', $user->branch_id)
                    ->whereIn('CYCLE_AWAL', CycleConfig::getAllowedDeployCycles());
            }

            return response()->json(
                R_TagihanDetail::collection($query->get()),
                200
            );
        } catch (\Throwable $e) {
