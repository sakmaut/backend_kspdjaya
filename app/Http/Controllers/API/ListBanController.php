    ...
    public function listBan() {
        ...
        try {
            ...
            DB::table('branch as a')
                ...
                ->chunk(1000, function ($results) use (&$build) {
                    foreach ($results as $result) {
                        $build[] = [
                            "KODE" => $result->CODE ?? '',
                            "CABANG" => $result->cabang ?? '',
                            "NO_KONTRAK" => $result->LOAN_NUMBER ?? '',
                            "NAMA PELANGGAN" => $result->customer_name ?? '',
                            ...
                            "JTH TEMPO AWAL" => isset($result->INSTALLMENT_DATE) ? date("d-m-Y", strtotime($result->INSTALLMENT_DATE)) : '',
                            "JTH TEMPO AKHIR" => isset($result->INSTALLMENT_DATE) ? date("d-m-Y", strtotime($result->INSTALLMENT_DATE)) : '',
