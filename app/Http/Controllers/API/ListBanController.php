...
class ListBanController extends Controller
{
                ...
                foreach ($arusKas as $item) {
                    if ($item->JENIS == 'PENCAIRAN') {
                        ...
                        $datas['datas'][] = [
                            ...
                            'amount' => number_format(round($getTttl, 2), 2, ',', '.'),
    ...
    public function listBanTest(Request $request)
    {
        try {
            ...
            if ($currentYear === $yearFromPayload) {
                $yearUsed = $now->year;
            } else {
                $yearUsed = $yearFromPayload;
            }
            ...
            $query1 = "SELECT  CONCAT(b.CODE, '-', b.CODE_NUMBER) AS KODE,
                        ...
                        FROM	credit_log_{$yearUsed} cl
                                ...
                                left join    k on cast(k.loan_number as char) = cast(cl.LOAN_NUMBER as char) and date_format(k.created_at,'%m%Y')='$dateFrom'
            ...
            if (strtolower($getPosition) != 'ho') {
                ...
                if (!empty($getBranch) && $getBranch != 'SEMUA CABANG' && $getBranch != '8593fd4e-b54e-11ef-97d5-bc24112eb731' && $getBranch != 'semua') {
